...
const sourceIcons: Record<SearchResult['source'], React.ReactNode> = {
  file: <FileText size={12} className="text-[var(--accent)]" />,
...
const sourceColors: Record<SearchResult['source'], string> = {
  file: 'bg-[var(--accent-subtle)] text-[var(--accent)]',
      ...
      <div className="px-3 py-3 border-b border-[var(--border)]">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg pl-9 pr-3 py-2 text-sm focus:outline-none focus:border-[var(--accent)]/50 placeholder-gray-500"
            autoFocus
            aria-label="Search documents"
          />
          {query && (
            <button onClick={() => setQuery('')} className="absolute right-2 top-2.5 text-[var(--text-muted)] hover:text-[var(--text-primary)]">
              <X size={14} />
            </button>
          )}
        </div>
        <div className="flex items-center justify-between mt-1.5">
          <span className="text-[10px] text-[var(--text-muted)]">{files.length} files indexed</span>
          <button
            onClick={initIndex}
            disabled={isRebuilding}
            className="flex items-center gap-1 text-[10px] text-[var(--text-muted)] hover:text-[var(--text-primary)] disabled:opacity-50"
          ...
          <div className="space-y-1">
            {results.map((r) => (
              ...
              >
                ...
                {r.snippet && (
                  <p className="text-[10px] text-[var(--text-muted)] mt-1 line-clamp-2">{r.snippet}</p>
                )}
                <div className="flex items-center gap-2 mt-1">
                  <div className="flex items-center gap-1">
                    <div className="h-1 w-12 bg-[var(--bg-hover)] rounded-full overflow-hidden">
                      <div
                    ...
                    <span className="text-[10px] text-[var(--text-muted)]">{r.relevance}%</span>
                  </div>
                  {r.date && <span className="text-[10px] text-[var(--text-muted)]">{formatDate(r.date)}</span>}
                </div>
              </button>
            ))}
          </div>
