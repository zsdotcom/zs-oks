  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 p-3 space-y-3 overflow-y-auto">
        ...
        <button onClick={handleSend} disabled={sending || !to.trim()} className="w-full py-2 bg-indigo-600 text-white text-xs rounded-lg hover:bg-indigo-700 disabled:opacity-50 flex items-center justify-center gap-1">
          <Send size={12} /> {sending ? 'Sending...' : 'Send Email'}
        </button>
        {currentFile && (
          <p className="text-[10px] text-gray-500">Attaching content from: {currentFile.name}</p>
        )}
      </div>
    </div>
  );
