...
```mermaid
flowchart TD
    Need{I need a library for...} --> Ask{Does it exist<br/>as a browser API?}

    Ask -->|Yes| Native[Use native API<br/>- BroadcastChannel<br/>- Web Workers<br/>- IndexedDB<br/>- Web Speech API]
    Ask -->|No| CDN{Can it load<br/>dynamically from CDN?}

    CDN -->|Yes| Dynamic[Dynamic import / script tag<br/>- Transformers.js<br/>- Orama<br/>- Mermaid<br/>- KaTeX<br/>- Leaflet]
    CDN -->|No| NPM{Is it react<br/>or react-dom?}

    NPM -->|Yes| Allow[Allowed as npm dep]
    NPM -->|No| Reject[Rejected - find alternative]

    style Native fill:#0b1326,color:#dae2fd
    style Dynamic fill:#1e293b,color:#94a3b8
    style Allow fill:#3b1a4b,color:#d0bcff
    style Reject fill:#4a1a1a,color:#ffb4ab
```
...
const { pipeline } =
  await import('https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.4.0/dist/transformers.min.js');
```
...
_Back to [Documentation Home](../index.md)_
