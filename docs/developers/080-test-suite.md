...
The test suite validates the 6-tier memory architecture, vector embedding pipeline, semantic search, LLM provider router, sandbox execution, and ICD-11 services with **74 test cases** across 6 files.
...
│   ├── geminiService.ts          # LLM provider router under test
│   ├── sandboxService.ts         # Sandbox execution under test
│   └── icd11Service.ts           # ICD-11 service under test
├── test/
...
│   └── icd11.test.ts             # 22 ICD-11 lookup tests
├── db/
│   └── indexedDB.ts
vitest.config.ts
```
...
| Storage | 3 | Estimate accuracy, maintenance purge, triggered maintenance |
...
- [Developer Guide: Development](040-development.md) — Testing strategy and running tests
- [Developer Guide: Memory Architecture](070-memory-architecture.md) — Memory tiers under test
- [Project Feature Status](../project/090-feature-status.md) — Test coverage metrics and status
- [Project Blueprint](../project/010-blueprint.md) — Success metrics coverage targets
